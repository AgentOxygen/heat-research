...
def aldente_spaghetti_differences(variable: str, exp: str, out_dir: str, pdf=None) -> None:
    ...
    for ds_name in max_all_datasets:
        ...
        data = data[f"{variable}_tx90pct"].mean(dim="lat").mean(dim="lon").dt.days
    ...
    for ds_name in min_all_datasets:
        data = xarray.open_dataset(ds_name)
        data = data[f"{variable}_tn90pct"].mean(dim="lat").mean(dim="lon").dt.days
    ...
    for ds_names, label in datasets_labels:
        ...
        for ds_name in ds_names:
            data = xarray.open_dataset(ds_name)
            if "max" in label:
                data = all_max_avg - data[f"{variable}_tx90pct"].mean(dim="lat").mean(dim="lon").dt.days
                if max_avg is None:
                    max_avg = data
                else:
                    max_avg += data
                ...
                data = all_min_avg - data[f"{variable}_tn90pct"].mean(dim="lat").mean(dim="lon").dt.days
                if min_avg is None:
                    min_avg = data
                else:
                    min_avg += data
